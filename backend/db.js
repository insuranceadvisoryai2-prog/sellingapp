...
export async function getAllProducts({ category, search, page = 1, limit = 24 } = {}) {
  let q = `SELECT * FROM products WHERE is_active=TRUE AND is_published=TRUE`;
  const params = [];
  let i = 1;
...
export async function countProducts({ category, search } = {}) {
  let q = `SELECT COUNT(*) FROM products WHERE is_active=TRUE AND is_published=TRUE`;
  const params = [];
  let i = 1;
  if (category) { q += ` AND category ILIKE $${i++}`; params.push(category); }
  if (search)   { q += ` AND (name ILIKE $${i} OR description ILIKE $${i} OR brand ILIKE $${i})`; params.push(`%${search}%`); i++; }
  ...
  const { rows } = await pool.query(
    `INSERT INTO products
      (name,description,price,original_price,discount_pct,image_url,images,
       category,subcategory,brand,specifications,stock,unit,min_order,source_url,source_site,is_published)
...
export async function saveManyProducts(products) {
  ...
  for (const p of products) {
    try { const r = await saveProduct(p); if (r) saved++; } catch {}
  }
  return saved;
}
...
export async function updateProduct(id, updates) {
  const allowed = [
    ...
    'specifications','stock','unit','min_order','is_active','is_published'
  ];
  const fields = [], values = [];
  let i = 1;
...
export async function getCategories() {
  const { rows } = await pool.query(
    `SELECT category, COUNT(*) as count FROM products
     WHERE is_active=TRUE AND is_published=TRUE AND category IS NOT NULL
     GROUP BY category ORDER BY count DESC`
  );
  return rows;
}

export async function getAllProductsAdmin({ page=1, limit=50 }={}) {
  const { rows } = await pool.query(
...
export async function addToCart(userId, productId, quantity) {
  ...
  await pool.query(
    `INSERT INTO cart_items (cart_id,product_id,quantity,unit_price)
     VALUES ($1,$2,$3,$4) ON CONFLICT (cart_id,product_id)
...
export async function updateCartItem(userId, productId, quantity) {
  ...
  if (quantity <= 0) {
    ...
    await pool.query(`UPDATE cart_items SET quantity=$1 WHERE cart_id=$2 AND product_id=$3`, [quantity, cart.id, productId]);
  }
  return getCartWithItems(userId);
}
...
// ── ORDERS ────────────────────────────────────────────────────────────────────
export async function createOrder(userId, { address, notes }={}) {
  ...
  try {
    ...
    const { rows:[order] } = await client.query(
      `INSERT INTO orders (user_id,total_amount,address,notes) VALUES ($1,$2,$3,$4) RETURNING *`,
      [userId, total, address, notes]
    );
    for (const item of items) {
      await client.query(
        `INSERT INTO order_items (order_id,product_id,product_name,quantity,unit_price) VALUES ($1,$2,$3,$4,$5)`,
        [order.id, item.product_id, item.name, item.quantity, item.unit_price]
      );
...
export async function createScrapeJob(site, query) {
  const { rows } = await pool.query(
    `INSERT INTO scrape_jobs (site,query) VALUES ($1,$2) RETURNING *`, [site, query]
  );
  return rows[0];
}
