...
// ── IMPORT TOKEN (for bookmarklet) ───────────────────────────────────────────
// Store import tokens in memory (persists until server restart)
const importTokens = new Map();

app.post('/api/admin/generate-import-token', adminMiddleware, (req, res) => {
  const token = require('crypto').randomBytes(32).toString('hex');
  const userId = req.user.id;
  importTokens.set(token, { userId, created: Date.now() });
  // Expire tokens after 30 days
  setTimeout(() => importTokens.delete(token), 30 * 24 * 60 * 60 * 1000);
  res.json({ token });
});
...
app.post('/api/admin/products/:id/approve', adminMiddleware, async (req, res) => {
  try {
    const p = await db.updateProduct(req.params.id, { approval_status: 'approved', is_published: true });
    res.json(p);
  } catch { res.status(500).json({ error: 'Failed' }); }
});
...
app.post('/api/admin/products/:id/reject', adminMiddleware, async (req, res) => {
  try {
    const p = await db.updateProduct(req.params.id, { approval_status: 'rejected', is_published: false });
    res.json(p);
  } catch { res.status(500).json({ error: 'Failed' }); }
});
...
app.post('/api/admin/import-product', async (req, res) => {
  ...
  // Accept either: JWT admin token OR import token
  ...
  if (!authorized && importTokens.has(token)) {
    authorized = true;
  }
