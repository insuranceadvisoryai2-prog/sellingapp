  ...
  return (
    <nav style={{ background:'#b71c1c', color:'white', position:'sticky', top:0, zIndex:100, boxShadow:'0 2px 8px rgba(0,0,0,0.3)' }}>
      <div className="container" style={{ display:'flex', alignItems:'center', height:60, gap:16 }}>
        ...
        <Link to="/products" style={{ color:'rgba(255,255,255,0.9)', fontSize:14, fontWeight:600, marginLeft:8 }}>Products</Link>
        <div style={{ flex:1 }} />
        {user ? (
          <div style={{ display:'flex', gap:12, alignItems:'center' }}>
            {user.role==='admin' && <Link to="/admin" style={{ color:'#ffcc02', fontWeight:700, fontSize:13 }}>Admin</Link>}
            ...
            <button onClick={handleLogout} style={{ background:'rgba(255,255,255,0.2)', color:'white', padding:'6px 14px', borderRadius:6, fontSize:13 }}>Logout</button>
          </div>
        ) : (
