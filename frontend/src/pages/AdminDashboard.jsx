...
export default function AdminDashboard() {
  ...
  const startScrape = async () => {
    ...
    try {
      ...
      toast(`✅ Scrape started! Job #${r.jobId} — check Pending tab in ~30s`);
      setScrapeQuery('');
      setTimeout(()=>{ loadJobs(); loadPending(); loadCount(); }, 20000);
      setTimeout(()=>{ loadJobs(); loadPending(); loadAll(); loadCount(); }, 60000);
        ...
        <div style={{ maxWidth:640 }}>
          <div className="card" style={{ padding:28, marginBottom:20 }}>
            ...
            <p style={{ color:'#757575', fontSize:13, marginBottom:20 }}>
              Paste a product URL to import it. After import, you can review and approve it in the <strong>Pending</strong> tab before it goes live.
            </p>
            ...
            {scrapeQuery && !inputIsUrl && (
              <p style={{ fontSize:12, color:'#e53935', marginBottom:12 }}>⚠️ Please paste a valid product URL (must start with https://)</p>
            )}
            {scrapeQuery && inputIsUrl && (
              <p style={{ fontSize:12, color:'#43a047', marginBottom:12, fontWeight:600 }}>✅ Valid URL detected — ready to import</p>
            )}
            ...
            <div style={{ background:'#e3f2fd', borderRadius:8, padding:16, marginTop:20 }}>
              <p style={{ fontWeight:700, color:'#1565c0', marginBottom:8 }}>How to import a product:</p>
              <ol style={{ fontSize:13, color:'#1a237e', paddingLeft:16, lineHeight:2 }}>
                <li>Go to the product page on the source website</li>
                <li>Copy the URL from the browser address bar</li>
                <li>Paste it in the box above and click Import</li>
                <li>Check the <strong>Pending</strong> tab to approve/reject</li>
                <li>Approved products go live in your store immediately</li>
              </ol>
            </div>
          </div>

          <div style={{ background:'#fff8e1', borderRadius:8, padding:16 }}>
            <p style={{ fontWeight:700, color:'#f57f17', marginBottom:6 }}>📝 Add products manually</p>
            <p style={{ fontSize:13, color:'#795548', marginBottom:10 }}>For full control over product details, images and pricing:</p>
