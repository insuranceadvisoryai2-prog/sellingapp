...
export default function Home() {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (search.trim()) nav(`/products?search=${encodeURIComponent(search)}`);
  };
  ...
  return (
    <div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #b71c1c 0%, #e53935 50%, #ff6f00 100%)', color: 'white', padding: '60px 16px', textAlign: 'center' }}>
        <h1 style={{ fontSize: 'clamp(24px,5vw,48px)', fontWeight: 800, marginBottom: 12 }}>
          India's #1 Wholesale Marketplace
        </h1>
        <p style={{ fontSize: 16, opacity: 0.9, marginBottom: 32 }}>
          Buy in bulk from Meesho, IndiaMart & more. Factory prices, direct to you.
        </p>
        <form onSubmit={handleSearch} style={{ display: 'flex', maxWidth: 560, margin: '0 auto', gap: 8 }}>
          ...
          />
          <button type="submit" className="btn btn-accent" style={{ padding: '14px 24px', borderRadius: 8, fontSize: 15 }}>
            Search
          </button>
        </form>
      </div>
      ...
      <div className="container" style={{ padding: '32px 16px 48px' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: 60, color: '#757575' }}>
            ...
            <p style={{ marginTop: 8 }}>Admin can add products via the scraper dashboard</p>
          </div>
        ) : (
